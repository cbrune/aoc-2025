...
fn prob2(prob_file: &str) -> Result<f64, Box<dyn std::error::Error>> {
    let points = data_init2(prob_file)?;
    let mut max_area = 0.;
    ...
    for (x0, y0, x1, y1, area) in areas {
        ...
        let contains = poly.contains(&rect);
        //        println!("contains: rect:{rect:?}, {contains}");
        // let intersects = poly.intersects(&rect);
        //        println!("intersects: rect:{rect:?}, {intersects}");

        // let inside = rect.iter().fold(true, |acc, e| acc && poly.intersects(e));

        if contains {
            println!("Found max area: {area} - ({x0}, {y0}), ({x1}, {y1}) - {contains}, {iter}");
            max_area = area;
            break;
        }
    }
